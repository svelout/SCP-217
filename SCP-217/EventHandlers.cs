...
namespace SCP_217
{
    public class EventHandlers
    {
        ...
        {      
            if (ev.Player.Role == RoleTypeId.Scp0492 && ev.Player.GameObject.GetComponent<RoleComponent>() != null)
            {
                Object.Destroy(ev.Player.GameObject.GetComponent<RoleComponent>());
            }
            if (ev.NewRole == RoleTypeId.Scp0492 && ev.Player.GameObject.GetComponent<RoleComponent>() != null)
            {
                ev.Player.ClearBroadcasts();
                ev.Player.Broadcast(3, Plugin.Instance.Translation.BroadcastMessage
                    .Replace("(COLOR)", ev.Player.Role.Color.ToHex())
                    .Replace("(BC)", "Теперь вы SCP-217-6"));
            }
        }
        ...
        {
            if (ev.Target != null && ev.Target.GameObject.GetComponent<RoleComponent>() != null)
            {
                if (ev.Player.CurrentItem.Type is ItemType.MicroHID)
                {
                    ev.Player.EnableEffect(EffectType.Deafened, 1f);
                }
            }
        }    
    }
